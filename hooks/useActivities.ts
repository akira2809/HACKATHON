...
export function useActivities(familyId?: string, options: QueryOptions = {}) {
    ...
    const queryChild = useActivitiesStore((state) => (queryKey ? state.queries[queryKey] : undefined));
    ...
    const resolvedQueryChild = queryChild ?? createQueryState(emptyActivities);
    ...
    useEffect(() => {
        ...
        if (sessionChanged || !resolvedQuery.isLoaded || !resolvedQueryChild.isLoaded) {
            if (!resolvedQuery.isLoading && !resolvedQueryChild.isLoading) {
                void fetchActivities(familyId, sessionVersion);
            }
        }
    }, [enabled, familyId, sessionVersion, fetchActivities, resolvedQuery.isLoaded, resolvedQuery.isLoading, resolvedQueryChild.isLoaded, resolvedQueryChild.isLoading]);
